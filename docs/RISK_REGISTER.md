...
| `R-S08` | Legal and Regulatory Risks | 16 `LEGAL-REVIEW-REQUIRED` items are enumerated but unresolved; no item has legal evidence on file. Four (data correction/deletion rights, retention periods, super-admin governance policy, owner-identity processing) block `IDA-4`'s citizen-facing surface directly | H/H | [`IDA4_LEGAL_GATE_MATRIX.md`](IDA4_LEGAL_GATE_MATRIX.md) (per-item tracking, all 16 `OPEN`) and [`IDA4_LEGAL_REVIEW_PACKAGE.md`](IDA4_LEGAL_REVIEW_PACKAGE.md) (counsel-facing engineering facts) now exist to support review; no legal review has occurred | OPEN — 16 items OPEN, 4 block IDA-4 |
| `R-S09` | Governance | A5 — the owner decision on which authentication path IDA-4 takes (wait for IDA-7's real auth, or authorise an IDA-4-scoped interim mechanism) — is unmade. Nothing sequencing after it (real auth, the deletion/correction mechanism, the ownership-transfer fraud model) can proceed until it is | H/H | `docs/IDA4_READINESS_AUDIT.md` §H names this the first item in the minimal unblock set; decision is the owner's alone | OPEN — decision pending |
