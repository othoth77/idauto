...
**Homepage « Rechercher par carte grise » (same pipeline, identify mode).** `web/citizen/home-registration.js` mounts the SAME scanner (`IdaRegistrationScanner.mount()` injects the dialog template) in `mode: "identify"`: capture → Scanic → optimisation → OCR happen in the browser, then the raw reads go to `POST /api/identify/registration-document` (`document-service.identify()`: parser → VIN | plate | make/model through the existing `VehicleResolver`, nothing written) for a signed-in user, or — for an anonymous visitor — the plate read from the card goes through the public plate route (no VIN, no make/model lookup without an account). The result card offers « Ouvrir le passeport »; a miss shows « Véhicule introuvable » with the plate prefill, châssis-number and make/model options, and creates nothing.
